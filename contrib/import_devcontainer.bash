...
# Executes the COPY
if [ -n "${FLAG_COPY}" ]; then
	cp -r "${origin_path}/devcontainer" "${target_path}/.devcontainer" || exit 1
	echo "> STATUS: Successfully copied .devcontainer folder."
fi

# Executes the LINK
if [ -n "${FLAG_LINK}" ]; then
	ln -snf "${origin_path}/devcontainer" "${target_path}/.devcontainer" || exit 1
	echo "> STATUS: Successfully linked .devcontainer folder."
fi
